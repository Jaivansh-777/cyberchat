...
function StatusViewer({ statuses, initialIndex, onClose }: {
  statuses: StatusItem[]
  initialIndex: number
  onClose: () => void
}) {
  ...
  const handleTap = (e: React.MouseEvent) => {
    ...
    if (x < rect.width / 2) { if (currentIndex > 0) setCurrentIndex(currentIndex - 1) }
    else { if (currentIndex < statuses.length - 1) setCurrentIndex(currentIndex + 1); else onClose() }
    setProgress(0); if (timerRef.current) clearInterval(timerRef.current)
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-black flex flex-col" onClick={handleTap}>
      ...
      <div className="flex-1 flex items-center justify-center px-8" onClick={(e) => e.stopPropagation()}>
        {current.imageUrl ? (
          <img src={current.imageUrl} alt="" className="max-w-full max-h-full rounded-2xl object-contain" />
        ) : (
      ...
      {current.content && current.imageUrl && (
        <div className="px-8 pb-12 text-center" onClick={(e) => e.stopPropagation()}>
          <p className="text-white/80 text-sm">{current.content}</p>
        </div>
      )}
    </motion.div>
  )
...
export default function StatusPage() {
  ...
  const [myStatusImage, setMyStatusImage] = useState<File | null>(null)
  const [myStatusPreview, setMyStatusPreview] = useState<string | null>(null)
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setMyStatusImage(file)
      setMyStatusPreview(URL.createObjectURL(file))
    }
    e.target.value = ''
  }
  ...
  const postStatus = async () => {
    ...
    if ((!text && !myStatusImage) || !userId) return

    try {
      let imageUrl = ''
      if (myStatusImage) {
        const formData = new FormData()
        formData.append('file', myStatusImage)
      ...
      setMyStatusText('')
      setMyStatusImage(null)
      setMyStatusPreview(null)
      setShowPost(false)
      loadStatuses()
    } catch {}
  }
  ...
  return (
    <div className="h-full bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/20 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-cyber">
        ...
        {otherStatuses.length > 0 && (
          <div className="px-4 pb-2">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3 px-1">Recent updates</h2>
            <div className="space-y-2">
              {Object.entries(grouped).map(([uid, items]) => {
                const latest = items.sort((a, b) => b.timestamp - a.timestamp)[0]
                return (
                  <motion.button key={uid} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        {otherStatuses.length === 0 && (
          <div className="flex flex-col items-center py-16 px-8 text-center">
            ...
            <p className="text-sm text-gray-400 max-w-xs">When your friends post a status, you&apos;ll see it here.</p>
          </div>
        )}
        <div className="h-8" />
      </div>
            ...
            <motion.div initial={{ y: 300 }} animate={{ y: 0 }} exit={{ y: 300 }}
              ...
              {myStatusPreview ? (
                <div className="relative mb-3">
                  <img src={myStatusPreview} alt="" className="w-full h-48 rounded-2xl object-cover" />
                  <button onClick={() => { setMyStatusImage(null); setMyStatusPreview(null) }}
              ...
              <textarea value={myStatusText} onChange={(e) => setMyStatusText(e.target.value)}
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="flex gap-2">
                  <button onClick={() => fileInputRef.current?.click()}
                    className="p-2.5 rounded-2xl bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors">
                ...
                <motion.button whileTap={{ scale: 0.85 }} onClick={postStatus}
                  disabled={!myStatusText.trim() && !myStatusImage}
      ...
      <AnimatePresence>
        {viewingIndex !== null && (
          <StatusViewer statuses={allViewable} initialIndex={viewingIndex} onClose={() => setViewingIndex(null)} />
        )}
      </AnimatePresence>
    </div>
  )
